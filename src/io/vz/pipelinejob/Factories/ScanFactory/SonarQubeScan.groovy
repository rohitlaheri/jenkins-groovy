...
public class SonarQubeScan implements ScanCode, Serializable {
    ...
    def scanSonar(Map param){
        ...
        steps.withSonarQubeEnv(sonarQubeEnv) {
         ...
         steps.sh "mvn clean -B verify mvn sonar:sonar -X -Dsonar.host.url=https://3665-173-71-125-24.ngrok.io -Dsonar.login=976b6274cf5f2bf30f80ca62064f1c0520032f6e -Dsonar.exclusions= **/*.java -Dsonar.coverage.exclusions=**/*.java -Dsonar.issuesReport.xml.enable=true -Dsonar.analysis.mode=publish -Dsonar.projectKey=prdecproj  -Dsonar.pullrequest.branch=${steps.env.gitlabBranch} -Dsonar.pullrequest.key=${steps.env.gitlabMergeRequestIid} -Dsonar.pullrequest.base=master -Dsonar.forceAuthentication=true"
