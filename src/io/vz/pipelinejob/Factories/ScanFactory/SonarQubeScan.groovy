...
public class SonarQubeScan implements ScanCode, Serializable {
    ...
    def scanSonarJava(Map param){
        ...
        steps.withSonarQubeEnv(sonarQubeEnv) {
            //steps.sh 'mvn clean package sonar:sonar -Dsonar.projectKey=my-aem-project -Dsonar.projectName=my-aem-project -Dsonar.issuesReport.xml.enable=true'
         steps.sh "mvn --batch-mode verify sonar:sonar -X -Dsonar.host.url=https://abe9-173-71-125-24.ngrok.io -Dsonar.login=976b6274cf5f2bf30f80ca62064f1c0520032f6e -Dsonar.projectKey=prdecproj -Dsonar.gitlab.url=https://gitlab.com/unifpip/jacoco-maven-unittestv2 -Dsonar.gitlab.user_token=glpat-NwZrsiL52kd8NypMZ1sQ -Dsonar.gitlab.commit_sha=${steps.env.gitlabMergeRequestLastCommit} -Dsonar.gitlab.project_id=${steps.env.gitlabMergeRequestTargetProjectId} -Dsonar.pullrequest.branch=${steps.env.gitlabBranch} -Dsonar.gitlab.ref_name=master -Dsonar.pullrequest.key=${steps.env.gitlabMergeRequestIid} -Dsonar.pullrequest.base=${steps.env.gitlabTargetBranch} -Dsonar.forceAuthentication=true -Dsonar.verbose=true  -Dsonar.coverage.jacoco.xmlReportPaths=target/site/jacoco/jacoco.xml -Dsonar.java.binaries=target/** -Dsonar.scm.forceReloadAll=true -Dsonar.gitlab.unique_issue_per_inline=true -Dsonar.language=java -Dsonar.issuesReport.xml.enable=true -Dsonar.gitlab.failure_notification_mode=status-code"
          // steps.sh "mvn --batch-mode verify sonar:sonar -Dsonar.host.url=https://abe9-173-71-125-24.ngrok.io -Dsonar.login=976b6274cf5f2bf30f80ca62064f1c0520032f6e -Dsonar.gitlab.commit_sha=${steps.env.gitlabAfter} -Dsonar.gitlab.ref_name=${steps.env.gitlabBranch} -Dsonar.gitlab.project_id=36610031 -Dsonar.branch.name=${steps.env.gitlabBranch} -Dsonar.gitlab.unique_issue_per_inline=true"
        }
    }
    /**
